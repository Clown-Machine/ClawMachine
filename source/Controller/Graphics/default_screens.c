...
void screenMoveUp()
{
    if (SELECTION > 0)
    {
        uint8_t offset = 20;
        uint8_t base = 20;
        blackenArea(20, base - 5 + offset * (SELECTION + 1), 10, 10);
        SELECTION -= 1;
    }
}
...
void screenMoveDown()
{
    if (SELECTION < 3)
    {
        uint8_t offset = 20;
        uint8_t base = 20;
        blackenArea(20, base - 3 + offset * (SELECTION + 1), 10, 10);
        SELECTION += 1;
    }
}
...
void screenMenuPosition()
{
    ...
    if (RELOAD_MENU)
    {
        ...
        if (current_mode == MANUAL_MODE || current_mode == SELECT_MODE)
        {
            writeToScreen("mode: MANUAL", SCREEN_WIDTH / 2, SCREEN_HEIGHT - 10,
                          CENTERED);
        }
        else if (current_mode == AUTOMATIC_MODE)
        {
            writeToScreen("mode: AUTOMATIC", SCREEN_WIDTH / 2,
            SCREEN_HEIGHT - 10,
                          CENTERED);
        }
    }
    writeToScreen("O", 20, base - 3 + offset * (SELECTION + 1), LEFT);
}
